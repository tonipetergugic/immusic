...
export default function HomePlaylistsDndList({
  moduleId,
  initialOrder,
  children,
}: {
  ...
  const childById = useMemo(() => {
    ...
    for (const child of childArray) {
      // HomePlaylistsDndItem wird als Element gerendert und hat props.id
  ...
  async function persist(nextOrder: string[]) {
    try {
      await fetch("/dashboard/admin/playlists/reorder", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ moduleId, orderedIds: nextOrder }),
      });
    } catch {
      // bewusst still — UI bleibt responsive, Persistenz kann später verbessert werden
    }
  }
