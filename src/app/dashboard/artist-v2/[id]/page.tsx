...
export default async function ArtistV2Page({
  ...
  const shareUrl = host
    ? `${proto}://${host}/dashboard/artist-v2/${artistId}`
    : `/dashboard/artist-v2/${artistId}`;
  ...
  const { data: profile, error: profileError } = await supabase
    .from("profiles")
    .select(
      "id, display_name, bio, location, instagram, tiktok, facebook, x, banner_url, avatar_url, updated_at"
    )
    .eq("id", artistId)
    .single();
